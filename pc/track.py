...
ballistics_calc = BallisticCalculator(resolution=(720, 1280))
...
def servo_controller(box):
    global ballistics_calc
    global ser
    global pan
    global tilt
    global one_in_ten_pan
    global one_in_ten_tilt

    center_coordinates = (int(box[0]+(box[2]-box[0])/2), int(box[1]+(box[3]-box[1])/2))
    # inversion
    center_coordinates = (center_coordinates[1], center_coordinates[0])
    angles = ballistics_calc.get_camera_angles(center_coordinates[0], center_coordinates[1])
    LOGGER.info('Angles: ' + str(angles))

    if abs(angles[0]) > 0:
        if one_in_ten_pan == 0:
            #pan
            pan = pan - int(angles[0])
            ser.write(bytes(str(pan) + 'p', 'utf-8'))
            one_in_ten_pan = 2
        else:
            one_in_ten_pan -= 1
    if abs(angles[1]) > 0:
        if one_in_ten_tilt == 0:
            # tilt
            tilt = tilt - int(angles[1])
            ser.write(bytes(str(tilt) + 't', 'utf-8'))
            one_in_ten_tilt = 2
        else:
            one_in_ten_tilt -= 1
def target_aquisition(outputs):
    target_list = []
    if len(outputs) > 0:
        for output in outputs:
            ...
            id = output[4]
            cls = output[5]
            target_list.append([bboxes, id, cls])

        servo_controller(engage_closest_target(target_list)[0])
    #for target in target_list:
    #    LOGGER.info(str(target[2]))
        # deschide o interfata in pyQt si lasa ma sa aleg o tinta

def engage_closest_target(target_list):
...
def parse_opt():
    ...
    parser.add_argument('--yolo-weights', nargs='+', type=Path, default=WEIGHTS / 'yolov5m6.pt', help='model.pt path(s)')
